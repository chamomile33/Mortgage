...
rooms = dcc.Checklist(['1', '2', '3', '4', '5', '6+', 'Студия', 'Свободная планировка'], inline=True,
                      labelStyle={'margin-right': '15px'}, inputStyle={'margin-right': '2px'}, id='rooms')
minprice = dbc.InputGroup(
    [dbc.Input(type='number', placeholder="от", min=0, max=900_000_000_000, id='minprice', step=1),
     dbc.InputGroupText("₽")])
maxprice = dbc.InputGroup(
    [dbc.Input(type='number', placeholder="до", min=0, max=900_000_000_000, id='maxprice', step=1),
     dbc.InputGroupText("₽")])
price = dbc.Row([
    dbc.Col(minprice),
    dbc.Col(maxprice)
...
minarea = dbc.InputGroup(
    [dbc.Input(type='number', placeholder="от", min=0, max=9000, id='minarea', step=1), dbc.InputGroupText("м²")])
maxarea = dbc.InputGroup(
    [dbc.Input(type='number', placeholder="до", min=0, max=9000, id='maxarea', step=1), dbc.InputGroupText("м²")])
area = dbc.Row([
    dbc.Col(minarea),
    dbc.Col(maxarea)
]
)
min_living_area = dbc.InputGroup(
    [dbc.Input(type='number', placeholder="от", min=0, max=9000, id='min_living_area', step=1),
     dbc.InputGroupText("м²")])
max_living_area = dbc.InputGroup(
    [dbc.Input(type='number', placeholder="до", min=0, max=9000, id='max_living_area', step=1),
     dbc.InputGroupText("м²")])
living_area = dbc.Row([
...
min_kitchen_area = dbc.InputGroup(
    [dbc.Input(type='number', placeholder="от", min=0, max=9000, id='min_kitchen_area', step=1),
     dbc.InputGroupText("м²")])
max_kitchen_area = dbc.InputGroup(
    [dbc.Input(type='number', placeholder="до", min=0, max=9000, id='max_kitchen_area', step=1),
     dbc.InputGroupText("м²")])
...
floor_type = dcc.Checklist(['Не первый', 'Не последний', 'Только последний'], inline=True,
                           labelStyle={'margin-right': '25px'}, inputStyle={'margin-right': '2px'}, id='floor_type')
floor = dbc.Row([dbc.Row([dbc.Col(minfloor), dbc.Col(maxfloor)]), dbc.Row(floor_type)])

min_house_year = dbc.InputGroup([dbc.Input(type='number', placeholder="от", min=1000, max=datetime.datetime.now().year,
                                           id='min_house_year', step=1), dbc.InputGroupText("г")])
max_house_year = dbc.InputGroup([dbc.Input(type='number', placeholder="до", min=1000, max=datetime.datetime.now().year,
                                           id='max_house_year', step=1), dbc.InputGroupText("г")])
house_year = dbc.Row([
    dbc.Col(min_house_year),
    dbc.Col(max_house_year)
]
)
mortgage_type = dcc.Dropdown(['Стандартная ипотека', 'Ипотека для семьи с детьми', 'Ипотека для ИТ специалистов'],
                             id='mortgage_type')
...
percent_on_mortgage = dbc.InputGroup(
...
html_form = html.Div(style={'margin-left': '10px'}, children=[
    ...
    dbc.Form(class_name='card p-1', style={'margin-top': '20px'}, children=[
        ...
        dbc.Row(
            [
                dbc.Row([dbc.Label("Процент от зарплаты, отдаваемый на ипотеку", width="auto",
...
app.layout = html.Div([
    ...
    dbc.Col(id='mortgage', style={'width': '60%', 'display': 'inline-block', 'margin-left': '10px',
...
def get_region_id(reg_name):
    regions_df = pd.read_csv('items.csv').set_index('name')
...
def get_url(form_dict):
    ...
    if form_dict['min_house_year'] is not None:
        basic_url += f"&min_house_year={form_dict['min_house_year']}"
    if form_dict['max_house_year'] is not None:
...
def get_figure(df):
    ...
    fig.update_layout(
        mapbox=dict(center=go.layout.mapbox.Center(lat=df['lat'].median(), lon=df['lon'].median()), zoom=7,
...
form = ['rooms', 'region', 'object_type', 'minprice', 'maxprice', 'minarea', 'maxarea', 'min_living_area',
        'max_living_area',
        'min_kitchen_area', 'max_kitchen_area', 'minfloor', 'maxfloor', 'floor_type', 'min_house_year',
        'max_house_year']
...
def get_map(clicks, rooms, region, object_type, minprice, maxprice, minarea, maxarea, min_living_area, max_living_area,
            ...
            mortgage_type, down_payment, salary, percent_of_mortgage):
    if clicks is not None:
        ...
        if mortgage_type == 'Ипотека для семьи с детьми':
            ...
            if object_type is None or object_type == 'Новостройка':
                loan_program = 1
            else:
                loan_program = 2
        if loan_program != 0:
